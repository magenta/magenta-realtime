...
def _build_tiny_system(seed: int = 0) -> MagentaRT2Sampler:
    """Build a tiny random-weight ``MagentaRT2Sampler`` for CI.

    Full-size codec construction is too slow for CI, so every nnx system
    test runs against this hand-built miniature (depthformer + SpectroStream
    wired end-to-end at toy dimensions).
    """
    rngs = nnx.Rngs(seed)
    num_codebooks = 4
    codebook_size = 8
