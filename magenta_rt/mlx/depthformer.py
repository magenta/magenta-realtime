...
from sequence_layers.mlx.typing import AnyPyTree, ArrayT, Float, Int, ScalarFloat, ScalarInt, typed

from typing import Any


def _mean_in_f32(x, axis):
  """Compute mean with float32 accumulation, matching JAX's jnp.mean behavior.

  MLX's mx.mean accumulates in the input dtype (e.g. bfloat16), which can cause
  significant precision loss. JAX upcasts to float32 for reduction internally.
  """
  original_dtype = x.dtype
  return mx.mean(x.astype(mx.float32), axis=axis).astype(original_dtype)
...
# MLX does not use structured PRNG keys like JAX.
# Use mx.array for RNG state (seeds).
PRNGKeyBatch = Any
...
def _tree_map(fn, tree):
  """Simple tree map for tuples/lists/dicts of mx.arrays."""
  if isinstance(tree, mx.array):
    return fn(tree)
  elif isinstance(tree, (sl.Sequence, sl.MaskedSequence)):
    return type(tree)(fn(tree.values), fn(tree.mask))
    ...
    return tree  # non-array leaf (e.g., int, None)
...
def split_batch_dim(tree: AnyPyTree, split_size: int) -> AnyPyTree:
  ...
  return _tree_map(split_array, tree)
...
def merge_batch_dim(tree: AnyPyTree) -> AnyPyTree:
  ...
  return _tree_map(merge_array, tree)
...
def interleave_sequences(
  ...
  if a is b and all(a is seq for seq in c):
    ...
    result = type(a)(
        mx.tile(
            a.values[:, mx.newaxis], (1, 2 + len(c)) + (1,) * (a.ndim - 1)
        ),
        mx.tile(a.mask[:, mx.newaxis], (1, 2 + len(c), 1)),
    )
  else:
    if (
    ...
    all_values = [mx.array(s.values) for s in [a, b, *c]]
    all_masks = [mx.array(s.mask) for s in [a, b, *c]]
    values = mx.stack(all_values, axis=1)
    mask = mx.stack(all_masks, axis=1)
    result = result_type(values, mask)

  return merge_batch_dim(result)


def get_large_negative_number(dtype: mx.Dtype) -> mx.array:
  ...
  dtype = mx.asarray(0, dtype=dtype).dtype
  # -0.7 is a float64 in Jax. Explicit cast output to target dtype.
  if mx.issubdtype(dtype, mx.inexact):
    dtype_max = mx.finfo(dtype).max
  elif mx.issubdtype(dtype, mx.integer):
    dtype_max = mx.iinfo(dtype).max
  else:
    raise ValueError('Unsupported dtype for inputs.')
  return mx.asarray(-0.7 * dtype_max, dtype=dtype)
...
def _sample_categorical_with_temperature(
  ...
  top_k = mx.array(top_k, mx.int32) if top_k is not None else None
  top_p = mx.array(top_p, mx.float32) if top_p is not None else None
  ...
  if classifier_free_guidance_scale is not None:
    ...
    classifier_free_guidance_scale = mx.array(
    ...
    if classifier_free_guidance_scale.ndim == 0:
      classifier_free_guidance_scale = mx.full(
          [batch_size, 1, 1],
          mx.array(classifier_free_guidance_scale, dtype=logits.dtype),
      )
  ...
  # logits tensor.
  if temperature.ndim == 1:
    temperature = temperature[:, mx.newaxis, mx.newaxis]
  if top_k is not None:
    if top_k.ndim == 1:
      top_k = top_k[:, mx.newaxis, mx.newaxis]
    else:
      assert top_k.ndim == 0
      top_k = top_k[mx.newaxis, mx.newaxis, mx.newaxis]
  if top_p is not None:
    if top_p.ndim == 1:
      top_p = top_p[:, mx.newaxis, mx.newaxis]
    else:
      assert top_p.ndim == 0
      top_p = top_p[mx.newaxis, mx.newaxis, mx.newaxis]

  # Generate Gumbel noise. The Gumbel-Max trick (argmax(logits + gumbel_noise))
  # is a way to sample from a categorical distribution.
  # Generate Gumbel noise using MLX random.
  # MLX mx.random.gumbel takes shape, not a key.
  gumbel_noise = mx.random.gumbel(logits.shape, key=rng_key[0]).astype(logits.dtype)

  def sample_logits_fn(logits: mx.array) -> mx.array:
    ...
    if valid_range is not None:
      ...
      indices = mx.arange(logits.shape[-1])[mx.newaxis, mx.newaxis]
      logits = mx.where(
    ...
    if top_k is not None:
      ...
      kth_logit = mx.take_along_axis(sorted_logits, -k, axis=-1)
      logits = mx.where(
          logits >= kth_logit,
          logits,
          get_large_negative_number(logits.dtype),
    ...
    if top_p is not None:
      ...
      cum_p = mx.cumsum(mx.softmax(sorted_logits, axis=-1), axis=-1)
      cutoff_indices = mx.sum(
          (cum_p < top_p).astype(mx.int32), axis=-1, keepdims=True
      )
      # Clip the indices to prevent out-of-bounds access if top_p is >= 1.0
      cutoff_indices = mx.minimum(cutoff_indices, logits.shape[-1] - 1)
      logits_threshold = mx.take_along_axis(
...
class Encoder(nn.Module):
  ...
  def __init__(self, config: Config):
    ...
    if config.embedding is not None:
      embedding_layers = [config.embedding]
    else:
      embedding_layers = [
          sl.Embedding.Config(
          ...
          sl.Scale.Config(math.sqrt(config.embedding_dimension)),
      ]
  ...
  def encode(
    ...
    if got_multi_tokens and not custom_embedding:
      inputs = inputs.apply_values(lambda v: mx.squeeze(v, axis=-1))
...
class MultivariateDecoder(sl.Emitting):
  ...
  class Config(sl.SequenceLayerConfig):
    ...
    def make(self):
      ...
      return MultivariateDecoder(self)

  def __init__(self, config: Config):
    super().__init__()
    self.config = config
    self.embedder = config.embedder.make(backend="mlx")
    self.temporal_body = config.temporal_body.make(backend="mlx")
    self.depth_body = config.depth_body.make(backend="mlx")
  ...
  def layer_with_emits(
    ...
    embedded: sl.Sequence = self.embedder.layer(x)

    # temporal_inputs has shape (B, T, D)
    temporal_inputs = embedded.apply_values(_mean_in_f32, axis=-2)[:, :-1]

    if self.config.temporal_input_dropout_prob > 0 and training:
      drop_example = mx.random.uniform(
          shape=(batch_size,) + (1,) * (temporal_inputs.ndim - 1)
      )
    ...
    if self.config.soft_cap_logits is not None:
      depth_outputs = depth_outputs.apply_values(
          lambda x: mx.tanh(x / self.config.soft_cap_logits)
  ...
  def get_initial_state(
    ...
    embedded = embedded.apply_values(_mean_in_f32, axis=-2)
    ...
    # MLX doesn't use JAX-style PRNG keys. We use a simple seed-based approach.
    # Create a batch of random keys for sampling.
    rng = mx.stack([mx.random.key(42 + i) for i in range(batch_size)])
    step = mx.zeros([batch_size], mx.int32)
  ...
  def step_with_emits(
    ...
    Args:
      ...
      forced_tokens: Optional array of shape (batch_size, 1, num_codebooks)
        containing tokens to force for this step. If provided and non-empty,
        the model skips the depth sampling loop and uses these tokens directly.
        (Note: This argument is specific to the MLX implementation and is not
        present in the JAX version.)
      training: Whether the model is in training mode.
      constants: Optional constants passed to the layers.
    ...
    embedded_frame: sl.Sequence = self.embedder.layer(
        previous_frame
    )
    # temporal_inputs has shape (B, 1, D)
    temporal_inputs = embedded_frame.apply_values(_mean_in_f32, axis=-2)
    ...
    if classifier_free_guidance_arity > 0:
      ...
      cfg_scale_batches = [mx.zeros(original_batch_size)] + [
          v[:: classifier_free_guidance_arity + 1]
          if mx.array(v).size == batch_size
          else v * mx.ones(original_batch_size)
          for _, v in sorted(classifier_free_guidance_scales.items())
      ]
      classifier_free_guidance_scale = mx.stack(
          cfg_scale_batches, axis=1
      ).flatten()
    else:
      classifier_free_guidance_scale = None

    # Only run the depth body for the first num_active_codebooks iterations,
    # then pad remaining codebooks with dummy (index-0) tokens.
    # This reduces latency by ~1.1ms per skipped codebook.
    num_active_codebooks = self.config.num_active_codebooks or self.config.num_codebooks
    ...
    for rvq_index in range(num_active_codebooks):
      ...
      depth_inputs = self.embedder.layer(depth_sample)

    # Pad remaining codebooks with dummy tokens (first token of each codebook).
    if num_active_codebooks < self.config.num_codebooks:
      batch = depth_samples[0].shape[0]
      for rvq_index in range(num_active_codebooks, self.config.num_codebooks):
        dummy_val = (
            self.config.num_reserved_tokens
            + rvq_index * self.config.codebook_size
        )
        dummy = mx.full(depth_samples[0].shape, dummy_val, dtype=depth_samples[0].dtype)
        depth_samples.append(dummy)

    depth_samples = mx.stack(depth_samples, axis=-1)
...
class EncoderDecoder(nn.Module):
  ...
  def __init__(self, config: Config):
    ...
    self.sampler = StreamingEncoderDecoderSampler(
        ...
        streaming_encoder=config.streaming_encoder,
        encoder_lookahead=0,
  ...
  #       constants=constants,
  #   )
  #   logits = logits.apply_values(lambda v: v.astype(mx.float32))

  #   loss = optax.softmax_cross_entropy_with_integer_labels(
  #       logits=logits.values, labels=emits['targets'].values
  #   )
  #   loss = sl.Sequence.from_values(mx.mean(loss, -1) / loss.shape[1])
...
class StreamingEncoderDecoderSampler(sl.SequenceLayer):
  ...
  def __init__(self, encoder, decoder, streaming_encoder, encoder_lookahead, conditioning_name=None):
    super().__init__()
    self.encoder = encoder
    self.decoder = decoder
    self.streaming_encoder = streaming_encoder
  ...
  def get_initial_state(
    ...
    zero_encoded = sl.Sequence(
        ...
        mx.zeros((batch_size, self.encoder.body.block_size), mx.bool_),
    )
    ...
    sampler_delay_countdown = mx.full(
        [batch_size], self.encoder_lookahead, mx.int32
    )
  ...
  def step(
    ...
    if classifier_free_guidance_scales:
      ...
      if not classifier_free_guidance_negatives:
        ...
        x_neg = [sl.Sequence(values=mx.zeros_like(x.values), mask=x.mask)] * len(
      ...
      is_repeatable = lambda v: (
          ...
          and mx.array(v).ndim >= 1
      )
      constants = {
          k: (
              _tree_map(lambda a: mx.repeat(a, arity, axis=0), v)
              if is_repeatable(v)
              else v
          )
    ...
    sampler_delay_countdown = mx.maximum(0, sampler_delay_countdown - 1)

    def broadcast_delay_active(ndim) -> mx.array:
    ...
    step_input = sl.MaskedSequence(
        mx.where(
            delay_active_values,
            mx.zeros_like(sampler_previous_output.values),
            sampler_previous_output.values,
        ),
        mx.where(
            delay_active_mask,
            mx.zeros_like(sampler_previous_output.mask),
