...
def branch_config(
  ...
  return sl.Parallel.Config(
      ...
      combination=sl.CombinationMode.MEAN,
      name=name,
  )
...
class MagentaRT2ModelBase(metaclass=abc.ABCMeta):
  ...
  compute_dtype: sl.DType = mx.bfloat16
  ...
  def target_tokens_config(self) -> TokensConfig:
    tt_config = dataclasses.replace(
        self.spectrostream,
        key='ss_target_tokens',
        frame_rate=SPECTROSTREAM.frame_rate,
    )
    if self.data_rvq_levels:
      tt_config = dataclasses.replace(tt_config, rvq_truncation_level=self.data_rvq_levels)
    return tt_config
  ...
  def depthformer_config(
    ...
    encoder_spec = dataclasses.replace(self.encoder_size, dropout_prob=self.dropout_prob, **kwargs)
    decoder_temporal_spec = dataclasses.replace(
        self.decoder_temporal_size, dropout_prob=self.dropout_prob, **kwargs
    )
    ...
    decoder_depth_spec = dataclasses.replace(
        self.decoder_depth_size, **depth_overrides
    )
    ...
    if self.use_pretrained_musiccoca_embedder:
      ...
      regular_embedder = transformer.MultiChannelEmbedding.Config(
          ...
          reduction_fn=lambda x, axis: mx.mean(x.astype(mx.float32), axis=axis).astype(x.dtype),
          param_dtype=self.param_dtype,
          compute_dtype=self.compute_dtype,
